...
window.onload = function() {
    ...
    //draw doodler(desenhar rabiscar)
    //context.fillStyle = "green";
    //context.fillRect(doodler.x, doodler.y, doodler.width, dood.height);
    ...
    velocitYlocityY = initialVelocityY;
    placePlatforms();
...
function update() {
  ...
  while (platformArray.length >  0 && platformArray[0].y >= boardHeight) {
    platformArray.shift(); //removes first elemente from the array
  ...
  if(gameOver) {
     context.fillText("Tente de novo pressionando 'espaço'", boardWidth/6, boardHeight*7/8);
  }
}
function moveDoodler(e) {
  if (e.code == "ArrowRight" || e.code == "KeyD") { //mode right
      velocityX = 4;
      doodler.Img = doodlerRightImg;
    }
    else if (e.code == "ArrowLeft" || e.code == "KeyA") { //mode left
      velocityX = -5;
...
function detectCollision(a, b) {
   return a.x < b.x + b.width &&  //(o canto superior esquerdo de a não alcança o canto superior direito de b)
          a.x + a.width > b.x && //(canto superior direito de a passa canto superior esquerdo de b)        
